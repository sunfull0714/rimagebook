...
sudo mkdir release
cd release
sudo cmake -D CMAKE_CXX_FLAGS_RELEASE="-fPIC" -D CMAKE_C_FLAGS_RELEASE="-fPIC" -D BUILD_SHARED_LIBS=OFF -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=${HOME}/usr/ -D VTK_DATA_ROOT=../../VTKData ..
sudo make -j 2
sudo make install
cd
